...
mysql(){
    ...
    echoGreen "开始编译安装！！" && tar -xf mysql-5.7.31.tar.gz && cd mysql-5.7.31  && cmake . -DCMAKE_INSTALL_PREFIX=/usr/local/mysql -DMYSQL_DATADIR=/usr/local/mysql/data -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_MEMORY_STORAGE_ENGINE=1 -DWITH_READLINE=1 -DMYSQL_UNIX_ADDR=/var/lib/mysql/mysql.sock -DWITH_BOOST=/usr/local/boost/ -DDOWNLOAD_BOOST=1 -DOWNLOAD_BOOST_TIMEOUT=1200 -DMYSQL_TCP_PORT=3306 -DENABLED_LOCAL_INFILE=1 -DWITH_PARTITION_STORAGE_ENGINE=1 -DEXTRA_CHARSETS=all -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci && make && make install
    ...
    cd /usr/local/mysql/support-files && cp mysql.server /etc/rc.d/init.d/mysql && mkdir /var/log/mariadb && mkdir /var/lib/mysql && touch /var/log/mariadb/mariadb.log && yes | cp my-default.cnf /etc/my.cnf && chkconfig --add mysql && chkconfig mysql on && chown -R mysql:mysql /var/log/mariadb/ && chmod 777  /var/lib/mysql && service mysql start
